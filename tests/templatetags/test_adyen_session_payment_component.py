...
def test_adyen_session_payment_component_no_styles(
    settings, mock_sessions_api_success
) -> None:
    """
    ...
    out = Template(
        "{% load adyen_tags %}"
        "{% adyen_payment_component language=adyen_language order=order  %}"  # noqa
    ).render(
        Context(
            {
...
def test_adyen_session_payment_component_with_styles(
    settings, mock_sessions_api_success
) -> None:
    """
    Test donation component with a valid campaign
